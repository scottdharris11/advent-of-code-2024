...
def parse_diskmap(s: str) -> DiskMap:
    """parse a diskmap from the supplied string"""
    entries = []
    file = True
    file_no = 0
    for c in s:
        blocks = int(c)
        if file:
            entries.append(File(file_no, blocks))
            file_no += 1
        else:
            entries.append(FreeSpace(blocks))
        file = not file
...
assert solve_part1(data) > 89093266248
